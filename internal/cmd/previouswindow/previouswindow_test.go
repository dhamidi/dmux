...
import (
	...
	"github.com/dhamidi/dmux/internal/cmd/previouswindow"
...
func TestExecAdvancesByMinusOne(t *testing.T) {
	c, ok := cmd.Lookup(previouswindow.Name)
	if !ok {
		t.Fatalf("previous-window not registered")
	}
	sess := fakeSessionRef{id: 3}
	win := fakeWindowRef{index: 0, name: "bash"}
	item := &fakeItem{session: sess, advanceRef: win}
	res := c.Exec(item, []string{previouswindow.Name})
	if !res.OK() {
