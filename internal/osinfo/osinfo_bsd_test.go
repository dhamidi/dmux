...
// exercise Client logic without real BSD sysctl calls.
...
func TestForegroundCommand_BSD(t *testing.T) {
...
func TestForegroundCommand_BSD_ErrorForUnknownPID(t *testing.T) {
	info := newFakeProcInfo()
	c := osinfo.New(info)
...
func TestForegroundCWD_BSD(t *testing.T) {
	info := newFakeProcInfo()
	info.cwds[42] = "/usr/local/src"
	c := osinfo.New(info)
	got, err := c.ForegroundCWD(42)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got != "/usr/local/src" {
		t.Errorf("ForegroundCWD: got %q, want %q", got, "/usr/local/src")
	}
}

func TestForegroundCWD_BSD_ErrorForUnknownPID(t *testing.T) {
	info := newFakeProcInfo()
	c := osinfo.New(info)
